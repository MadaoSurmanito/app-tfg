...
\begin{itemize}
    ...
    \item Next.js 16.1.6
    \item React 19.x
    \item next-pwa 5.x (integración como Progressive Web App)
    \item cloudflare/cloudflared (ejecutado mediante Docker para túnel HTTPS de pruebas)
    \item PostgreSQL 16 (ejecutado en contenedor Docker)
    \item Docker Desktop 4.62.0
...
\subsubsection*{Configuración y pruebas como Progressive Web App (PWA)}

Con el objetivo de probar la aplicación en dispositivos móviles como aplicación instalable, se configuró el proyecto como Progressive Web App (PWA) mediante el paquete \texttt{next-pwa}:

\begin{lstlisting}[language=bash]
npm install next-pwa
\end{lstlisting}

Para permitir su instalación en dispositivos móviles es necesario ejecutar la aplicación en modo producción:
...
En Android, la instalación se realiza desde cualquier navegador que no sea Google Chrome mediante la opción \textit{Instalar aplicación}.  
En iOS, la instalación debe realizarse desde Safari seleccionando \textit{Añadir a pantalla de inicio}.

\subsubsection*{Túnel HTTPS para pruebas móviles}

Para poder instalar la aplicación como PWA en dispositivos móviles, es necesario servirla bajo HTTPS. Durante el desarrollo se utilizó Cloudflare Tunnel ejecutado como contenedor Docker, evitando instalaciones adicionales en el sistema.
...
El contenedor genera una URL HTTPS temporal que permite acceder a la aplicación desde el smartphone y realizar el proceso de instalación para pruebas.
